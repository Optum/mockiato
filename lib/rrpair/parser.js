      ...
      fs.readdir(path, (err, files) => {
        ...
        if (numberOfFiles % 2 !== 0)
          ...
          files.forEach( function( file, index ) {
            if(file.endsWith(constants.REQ_FILE_END) || file.endsWith(constants.RSP_FILE_END)){//by passing if
            ...
            if (err) return reject(err);
              ...
              if (filename.endsWith(constants.REQ_FILE_END)) {
                ...
                if (uri.includes(constants.QUESTION_MARK)) {
                  ...
                  for (var i = 0; i < seperateQueryString.length; i++) {
                ...
                rr.reqData = JSON.parse(obj.body);
                    ...
                    if(!firstWord2.startsWith(constants.SOAP_FILE_STARTSWITH)){
                      return reject(new Error(constants.NOT_SOAP_TYPE_ERR_MSG));
                    }
                ...
                rr.reqData = content;    
                  ...
                  rr.resData = JSON.parse(obj.body);
                    ...
                    if(!firstWord4.startsWith(constants.SOAP_FILE_STARTSWITH)){
                      return reject(new Error(constants.NOT_SOAP_TYPE_ERR_MSG));
                    }
                  rr.resData = content; 
                }

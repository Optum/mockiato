...
function parse(path, type) {
  ...
  return new Promise(function (resolve, reject) {
      ...
      fs.readdir(path, (err, files) => {
        ...
        if(files.includes("__MACOSX")) isMacZip=true;

        if (numberOfFiles % 2 !== 0 && !isMacZip)
          return reject(new Error(constants.NOT_EVEN_ERR_MSG));

          if (isMacZip && numberOfFiles % 2 == 0)
          return reject(new Error(constants.NOT_EVEN_ERR_MSG));
          ...
          files.forEach( function( file, index ) {
            ...
            if(match[1].endsWith(constants.REQ_FILE_END) || match[1].endsWith(constants.RSP_FILE_END)){
              ...
              }else if(match[1] !== '__MACOSX') {return reject(new Error(constants.WRONG_FILE_ENDING_OR_TYPE_ERR_MSG));}
          });
          ...
          dir.readFiles(path, function (err, content, filename, next) {
            ...
            if (isMacZip &&  fileCounter === numberOfFiles-1) {
              return resolve(serv);
            }
            else if(!isMacZip && fileCounter === numberOfFiles) {
              return resolve(serv);
            }

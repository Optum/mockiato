...
function parse(spec) {
  return new Promise(function(resolve, reject) {
    ...
    // try to pull base path
    if (spec.servers.length) {
      for (let j = 0; j < spec.servers.length; j++) {
        let urlStr = spec.servers[j].url;
        if (!url.parse(urlStr).protocol) continue;
    ...
    if (!serv.basePath) serv.basePath = '/api';
      ...
      methods.forEach(function(method, index) {
        ...
        responses.forEach(function(resp) {
          ...
          if (params) {
            ...
            params.forEach(function(param) {
              ...
              if (param.schema && param.schema.type) {
                ...
                let reg = param.$ref.match(/s\/.+\//);
                let str = reg.toString().replace(/s/, '');
                let str2 = str.replace(/\//g, '');

                let componentName = param.$ref.replace('#/components' + str, '');
                let component = spec.components[str2][componentName];

                let paramSchema = component.schema;
                if (paramSchema) {
                  defVal = processSchema(paramSchema);
                  switch(component.in) {
                    case 'query':
                      rr.queries[component.name] = defVal;
                      break;
                    case 'header':
                      rr.reqHeaders[component.name] = defVal;
                      break;
                    default:
                      debug(`Warning - Could not virtualize parameter ${component.name} in ${component.in}`);
                  }
                }
              }
            });
          ...
          function processSchema(schema) {
            ...
            if (schema.content) {
              if (!rr.payloadType) {
                setPayloadType(rr, 'application/json');
              }
